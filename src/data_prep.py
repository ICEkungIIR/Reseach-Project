...
def load_raw(task: str):
    ...
    For wisesight_sentiment, each item: {"text": str, "label": str}
    For thai_ner, each item: {"tokens": List[str], "ner_tags": List[str]}

    Also caches the raw pull to data/raw/{task}.jsonl so we're not
    solely dependent on the HF cache (dataset versions on the hub can
    change later).
    """
    ...
    # Datasets on the hub sometimes ship pre-split; we deliberately
    # concatenate everything back together here because we do our OWN
    # fixed-seed split below, applied identically across tasks/models.
    all_examples = []
...
def clean_text(examples: list, task: str) -> list:
    ...
    for ex in examples:
        if task == "wisesight_sentiment":
            text = ex.get("text", "") or ex.get("texts", "")
            ...
            if key in seen:
                continue
            seen.add(key)
            cleaned.append({"text": text, "label": ex["label"] if "label" in ex else ex.get("category")})

        elif task == "thai_ner":
            tokens = ex.get("tokens") or ex.get("words")
            tags = ex.get("ner_tags") or ex.get("tags")
            if not tokens or not tags or len(tokens) != len(tags):
                continue
            # Drop examples that are just whitespace-only tokens end to end
...
def update_data_card(task: str, train: list, val: list, test: list):
    """
    Append real stats for this task to data/DATA_CARD.md, replacing the
    placeholder row for it in the datasets table if present, and adding
    a stats section underneath.
    """
